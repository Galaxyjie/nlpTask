...
class TextRNN(nn.Module):
    def __init__(self):
        ...
        self.W_aa_l = [nn.Linear(n_hidden, n_hidden, bias=False)
                       ] * (num_layer - 1)
        self.W_aa = [nn.Linear(n_hidden, n_hidden, bias=False)] * num_layer
        self.b_a = [nn.Parameter(torch.ones([n_hidden]))] * num_layer
        '''end'''
    ...
    def forward(self, X):
        ...
        # 初始化隐藏层状态全0
        m_a = [torch.zeros([sample_size, n_hidden])] * num_layer
        #第一层
        #a^(t) = tanh(W_{ax}x^(t)+W_{aa}a^(t-1)+b_{a})
        for x in X:
            #第一层
            m_a[0] = self.tanh(
                self.W_ax(x) + self.W_aa[0](m_a[0]) + self.b_a[0])
            #后面的层 w_aa_l [n_hidden,n_hidden]
            for layer in range(1, num_layer):
                m_a[layer] = self.tanh(self.W_aa_l[layer - 1](m_a[layer - 1]) +
                                       self.W_aa[layer](m_a[layer]) +
                                       self.b_a[layer])

        # model : [batch_size, n_class]
        #y^(t)=softmx(Wa^(t)+b)
        model_output = torch.softmax(self.W(m_a[num_layer - 1]) + self.b,
                                     dim=0)
        '''end'''
        return model_output
        ...
        for input_batch, target_batch in zip(all_input_batch,
            ...
            if (count_batch + 1) % 50 == 0:
                print('Epoch:', '%04d' % (epoch + 1), 'Batch:',
                      '%02d' % (count_batch + 1), f'/{batch_number}', 'lost =',
...
def test_rnnlm(select_model_path):
    ...
    print('lost =', '{:.6f}'.format(total_loss / count_loss), 'ppl =',
          '{:.6}'.format(math.exp(total_loss / count_loss)))
...
if __name__ == '__main__':
    ...
    word2number_dict, number2word_dict = make_dict(
        train_path)  #use the make_dict function to make the dict
